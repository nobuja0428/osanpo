...
export default function SupportService() {
  return <main id="main" className="business-page"><BusinessStructuredData name={service.title} description={description} path="business/support/" /><BusinessPageView contentId="support" serviceType="support" eventName="service_view" />
    ...
    <section className="section"><div className="narrow business-service-content"><p className="business-price">参考価格 {service.price}</p><p>{service.priceNote}</p><h2>公開後に困らないための小さな支え</h2><p>更新の頻度や内容は、お店ごとに異なります。必要なときだけ相談できるよう、対応範囲を先に確認してから進めます。</p><h2>相談できる内容の例</h2><ul>{service.deliverables.map((item) => <li key={item}>{item}</li>)}</ul><div className="business-callout"><h2>月額の目安</h2><p>月額5,000円・10,000円・15,000円を目安とし、実際の対応範囲は事前の相談で決めます。対応時間や成果を約束するものではありません。</p></div><BusinessContactCta serviceType="support" placement="support-bottom" /></div></section>
  </main>;
}
