...
export function BusinessServiceCards() {
  return <div className="business-card-grid">{businessServices.map((service) => <article className="business-card" key={service.id}>
    ...
    <Link href={service.href} className="text-link business-card-link" data-analytics-event="business_cta_click" data-page-type="business" data-content-id={service.id} data-placement="business-service-list">詳しく見る →</Link>
  </article>)}</div>;
}
