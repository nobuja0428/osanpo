...
export function BusinessServiceCards() {
  return <div className="business-card-grid">{businessServices.map((service) => <article className="business-card" key={service.id}>
    ...
    <Link href={`/business/${service.id}/`} className="text-link" data-analytics-event="service_view" data-page-type="business" data-content-id={service.id} data-service-type={service.id} data-placement="business-service-list">詳しく見る →</Link>
  </article>)}</div>;
}
